...
//      gameMap[m][n].curState = DIGGED;
      gameMap[m][n].curState = BlockState(gameMap[m][n].valueFlag);
   }
...
//             gameState = FAULT;
   ...
   else if (markNumber == totalMineNumber)
   {
      if(gameMap[m][n].curState == MARKED || gameMap[m][n].curState == WRONG_BOMB)
      {
         gameMap[m][n].curState = UN_DIG;
         gameState = PLAYING;
         markNumber--;
      ...
      else if(gameMap[m][n].curState == UN_DIG)
      {
          gameState = FAULT;
          QMessageBox::information(NULL,"Mark error!", QObject::tr("Mark number is up to limit %1.").arg(totalMineNumber));
      }
   }
   // 检查游戏输赢,并作调整
   checkGame();
...
void GameModel::leftRight(int m, int n)
{
   if (gameMap[m][n].curState > BlockState(0) && gameMap[m][n].curState < BlockState(9))
   {
       int rightCheck = 0;
       int wrongCheck = 0;
       for (int r = -1; r <=1; r++)
          for (int c = -1; c <= 1; c++)
          {
             if (m + r >= 0 && m +r < mRow &&
                 ...
                 !(r == 0 && c == 0))
             {
                if (gameMap[m+r][n+c].curState == MARKED)
                {
                   rightCheck++;
                   wrongCheck++;
                }
                else if (gameMap[m+r][n+c].curState == WRONG_BOMB)
                       wrongCheck++;
             }
          }

       if (gameMap[m][n].valueFlag == rightCheck)
       {
           for (int r = -1; r <=1; r++)
              for (int c = -1; c <= 1; c++)
              {
                 if (m + r >= 0 && m +r < mRow &&
                     n + c >= 0 && n + c < mCol &&
                     !(r == 0 && c == 0) &&
                     gameMap[m + r][n + c].curState == UN_DIG)
                 digMine(m + r, n + c);
              }
       }
       else if (gameMap[m][n].valueFlag == wrongCheck)
           gameState = OVER;
   }
   checkGame();
}
